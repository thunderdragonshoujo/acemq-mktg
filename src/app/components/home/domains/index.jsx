  ...
  <section className="hidden sm:flex px-[3rem] sm:px-[10rem] gap-x-[4rem]">
    <div className="flex relative">
      <img
        src="/mille_3.svg"
        alt="arrow"
        className="hidden absolute mt-[4rem] sm:w-[4rem] sm:block"
      />
      <div className="flex flex-col sm:ml-[7rem] ">
        <div className="">
          ...
          <p className="">
            AceMQ is a subsidiary of a larger, highly specialized full-stack
            services company, Ace8, therefore we understand how your Message
            Queing tool fits into your environment as a whole and thus we can
            advise and support you beyond the scope of your tool of choice.
          </p>
        </div>
        ...
        <div className="h-[93rem] grid place-content-center relative">
          ...
          />
          {domains.map((domain, index) => (
            <DomainDesktop domain={domain} key={index} id={index} />
          ))}
        </div>
        <button className="px-[2.5rem] border border-[#FF6600] self-center mt-[4rem]">
          Ace8 Services
        </button>
        <Banner />
      </div>
...
const DomainMobile = ({ domain, id }) => (
  <div
    className={`w-[230px] flex flex-col ${id % 2 ? "self-end" : "self-start"}`}
  >
    <Image
      src={domain.image}
      width={115}
    ...
    />
    <p
      className={`text-[1.4rem] font-[700] ${
        id % 2 ? "text-right" : "text-left"
      }`}
    >
      {domain.name}
    </p>
    <p
      className={`text-[1.6rem] font-[700] text-[#FF6600] ${
        id % 2 ? "text-right" : "text-left"
      }`}
    >
      {domain.title}
...
const Banner = () => (
  <div className="border border-[#FF6600] p-[1rem] sm:p-[3rem] rounded-[.8rem] mt-[3rem] sm:flex sm:justify-between sm:mt-[9rem]">
    <div className="flex flex-col sm:w-[58rem]">
      <h3 className="font-[400] text-[1.4rem] text-center sm:text-left mb-[1.3rem]">
        <span className="font-[700] text-[#FF6600]">RabbitMQ</span> Enterprise{" "}
        <span className="font-[700]">support</span>
      </h3>
      <h3 className="text-center text-[2.7rem] sm:text-[3.5rem]  font-[700] sm:text-left">
        How we <span className="font-[400]">Saved Millions</span> in{" "}
        <span className="text-[#FF6600]">Downtime</span> for a{" "}
        <span className="text-[#FF6600]">Global</span>{" "}
        <span className="font-[400]">Retail Giant</span>
      </h3>
      <p className="hidden sm:block">
        Learn how AceMQ’s RabbitMQ Enterprise support saved a global Retail
        Giant millions of dollars in downtime, while saving them thousands in
        hiring costs.
      </p>

      <Link href="#contact">
        <button className="border border-[#FF6600] self-center sm:self-start mt-[2rem]">
          Learn More
        </button>
      </Link>
    </div>
